...
function scheduleUpdate(config, packageName, latestVersion, mode) {
  const spinner = ora(mode === 'auto' ? '자동 업데이트 준비 중...' : '업데이트 준비 중...').start();
  ...
  try {
    ...
    spinner.succeed(chalk.green(mode === 'auto' ? '자동 업데이트를 시작합니다.' : '업데이트를 시작합니다.'));

    console.log(boxen(
      chalk.green.bold(mode === 'auto' ? '자동 업데이트 시작\n\n' : '업데이트 시작\n\n') +
    ...
    spinner.fail(chalk.red(mode === 'auto' ? '자동 업데이트 시작 실패' : '업데이트 시작 실패'));
...
export async function checkForUpdates(packageName) {
  ...
  if (shouldUpdate.update) {
    return scheduleUpdate(config, resolvedPackageName, latestVersion, 'manual');
  }
...
export async function checkForUpdatesQuietly(packageName) {
  try {
    ...
    if (shouldAttemptAutoUpdate(updateConfig, latestVersion, now)) {
      return scheduleUpdate(config, resolvedPackageName, latestVersion, 'auto');
    }
